...
class ShowMoreTableViewController: UITableViewController {
    var showMoreArr: [Person] = []
    
    @IBOutlet var showMoreTableView: UITableView!
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return showMoreArr.count
    }
    
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "contactCell", for: indexPath) as? HomeContactCell {
            cell.nameLabel.text = showMoreArr[indexPath.row].name
            cell.profileImageView.image = showMoreArr[indexPath.row].image
            return cell
        }
        ...
        if segue.identifier == "showMoreContactDetail" {
            if let destination = segue.destination as? ContactDetailTableViewController {
                ...
                if indexPath.section == 0 {
                    destination.selectedContact = showMoreArr[indexPath.row]
